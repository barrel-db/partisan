...
handle_call({emit, Node, ServerRef, Message},
    ...
    ?INFO("Emitting message with clock: ~p", [LocalClock]),
...
handle_call({receive_message, {causal, Label, _Node, _ServerRef, _IncomingOrderBuffer, MessageClock, _Message}=FullMessage},
    ...
    ?INFO("Received message ~p and inserting into buffer.", [MessageClock]),
    ...
    case DeliveryFun of
        undefined ->
            try
                ...
                _:Error ->
                    ?DEBUG("Error forwarding message ~p to process ~p: ~p", [Message, ServerRef, Error])
            end;
        DeliveryFun ->
...
internal_receive_message({causal, _Label, _Node, ServerRef, IncomingOrderBuffer, MessageClock, Message}=FullMessage,
    ...
    ?INFO("Attempting delivery of messages: ~p", [MessageClock]),

    case orddict:find(MyNode, IncomingOrderBuffer) of
        %% No dependencies.
        error ->
            ?INFO("Message ~p has no dependencies, delivering.", [MessageClock]),
        ...
        {ok, DependencyClock} ->
            case partisan_vclock:dominates(LocalClock, DependencyClock) of
                %% Dependencies met.
                true ->
                    ?INFO("Message ~p dependencies met, delivering.", [MessageClock]),
                    deliver(State0#state{buffered_messages=BufferedMessages -- [FullMessage]}, IncomingOrderBuffer, MessageClock, ServerRef, Message);
                %% Dependencies NOT met.
                false ->
                    %% Buffer, for later delivery.
                    ?INFO("Message ~p dependencies ~p NOT met.", [MessageClock, DependencyClock]),
